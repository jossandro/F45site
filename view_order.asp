...
function postcode_change(postcode, address){
      document.form2.postcode.value = postcode;
      document.form2.address.value = address;
      document.form2.submit();
}
function Promo_change(Promo_Code){
      document.form3.Promo_Code.value = Promo_Code ;
      document.form3.submit();
}

function MM_validateForm() { //v4.0
  if (document.getElementById){
    var i,p,q,nm,test,num,min,max,errors='',args=MM_validateForm.arguments,message;
    for (i=0; i<(args.length-2); i+=3) { 
      test=args[i+2]; 
      message = args[i+1];
      val=document.getElementById(args[i]);
      if (message == "") nm=val.name;
      else nm=message; 
